...
class SuperPointExtractor:
    ...
    def extract(self, image_bgr: np.ndarray, max_corners: int | None = None) -> FrameFeatures:
        """Mirrors DiskExtractor.extract's interface -- `image_bgr` is either
        an 8-bit BGR image or an already-normalized float [0, 1] image (same
        BGR channel order either way); `max_corners`, if given, overrides
