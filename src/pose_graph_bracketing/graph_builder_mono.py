...
monocular (essential-matrix) frontend.

Mirrors graph_builder.PoseGraphBuilder's structure (ISAM2 backend, motion prior,
diagnostics) but replaces stereo triangulation + PnP with 2D-2D essential-matrix
VO (odometry.estimate_pose) and a rotation + translation-direction-only factor
(factors.make_vo_direction_factor) -- monocular VO cannot recover metric scale,
so the graph's overall scale is only weakly resolved via the motion-prior
velocity states.
"""
...
from pose_graph_bracketing.odometry import estimate_pose
...
class MonoPoseGraphBuilder:
    ...
    def _evict_old_features(self, current_idx: int) -> None:
        ...
        for k in [k for k in self._feature_cache if k < cutoff]:
    ...
    def _try_vo_factor(
        ...
        pts_j = feats_j.keypoints[match.indices_a]
        pts_i = feats_i.keypoints[match.indices_b]

        est = estimate_pose(pts_j, pts_i, self.calib.K, self.calib.dist, self.cfg.odometry, shape_i)
        if est is None or not est.reliable:
            if visual_matches is not None:
                reason = "vo_failed" if est is None else "unreliable_" + "_".join(w.split()[0] for w in est.warnings)
                for idx_a_raw, idx_b_raw in zip(match.indices_a, match.indices_b):
                    visual_matches.setdefault(j, []).append(
                        MatchRecord(int(idx_a_raw), int(idx_b_raw), kept=False, reason=reason)
                    )
            return None

        if visual_matches is not None or keypoint_status is not None:
            for k, (idx_a_raw, idx_b_raw) in enumerate(zip(match.indices_a, match.indices_b)):
        ...
        return make_vo_direction_factor(
            _pose_key(j),
            _pose_key(i),
            est.R,
            est.t,
            vo_noise_model(self.cfg.odometry.rotation_sigma, self.cfg.odometry.translation_direction_sigma),
        )
    ...
    def process_frame(self, idx: int, frames: list[FrameInfo]) -> FrameResult:
        ...
        # Ensure features for this frame are ready.
        ...
        if self._video is not None:
            ...
            keypoint_status = {kp_idx: "no_depth" for kp_idx in range(len(feats_i.keypoints))}
            visual_matches = {}
